    ...
    <>
      ...
      <section className="section-pad max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {posts.map((post, i) => (
            <article key={i} className="group cursor-pointer bg-[#020f0c] rounded-2xl p-8 border border-[rgba(255,255,255,0.05)] hover:border-[rgba(10,173,146,0.3)] transition-colors duration-300">
              <div className="flex items-center gap-4 mb-4">
                <span className="text-xs font-semibold uppercase tracking-widest text-[#0aad92]">{post.category}</span>
                <time className="text-xs text-white/40" dateTime={post.date}>{post.date}</time>
              </div>
              <h2 className="text-2xl font-bold mb-4 group-hover:text-[#0aad92] transition-colors" style={{ fontFamily: "var(--font-fraunces)" }}>
                {post.title}
              </h2>
              <p className="text-white/60 mb-6 line-clamp-2">
                Expert insights and actionable strategies to help your brand grow smarter and reach further in today&apos;s digital landscape.
              </p>
